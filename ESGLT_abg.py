...
def model():
    """ Acausal bond graph ESGLT_abg.py
    Created by svgBondGraph at Fri Dec 18 12:28:17 2020 from ESGLT_abg.svg

    Usage:
    import ESGLT_abg; model = ESGLT_abg.model()
    """

    model = bgt.new(name="ESGLT")
    ...
    bgt.connect(BGT7,(r56,0))
    bgt.connect((r56,1),BGT2)
    bgt.connect(BGT2,BGT1)
    bgt.connect(BGT1,Ci)
    bgt.connect((r61,1),BGT3)
    bgt.connect(BGT3,BGT4)
    bgt.connect(BGT4,BGT22)
    bgt.connect(BGT22,(r12,0))
    bgt.connect((r12,1),BGT6)
    bgt.connect(BGT4,Co)
    bgt.connect(BGT6,BGT15)
    bgt.connect(BGT9,BGT7)
    bgt.connect((r25,1),BGT11)
    bgt.connect((r34,1),BGT12)
    bgt.connect(BGT8,(r45,0))
    bgt.connect((r45,1),BGT10)
    bgt.connect(BGT10,BGT9)
    bgt.connect(BGT11,BGT9)
    bgt.connect(BGT10,BGT20)
    bgt.connect(BGT8,SCNai)
    bgt.connect(BGT9,CNai)
    bgt.connect(BGT12,BGT8)
    bgt.connect(BGT13,(r25,0))
    bgt.connect(BGT14,(r34,0))
    bgt.connect(BGT16,(r23,0))
    bgt.connect((r23,1),BGT17)
    bgt.connect(BGT19,BGT16)
    bgt.connect(BGT15,BGT16)
    bgt.connect(BGT15,BGT13)
    bgt.connect(BGT15,CNao)
    bgt.connect(BGT17,SCNao)
    bgt.connect(BGT17,BGT14)
    bgt.connect(BGT5,E)
    bgt.connect(BGT18,Nao)
    bgt.connect(BGT19,So)
    bgt.connect(BGT20,Si)
    bgt.connect(BGT21,Nai)
    bgt.connect(BGT2,BGT21)
    bgt.connect(BGT2,BGT21)
    bgt.connect(BGT18,BGT22)
    bgt.connect(BGT18,BGT22)
    bgt.connect(BGT0,BGT5)
    bgt.connect(BGT6,BGT5)
